...
const Edit = ({ isOpen, onClose, onFormSubmit, user }) => {
  ...
  const sendData = async (e) => {
    e.preventDefault();
    const { UserId, Name, LastName, Email, UserName, Password, Role } = userData;
    // Primero, obtén el nombre de la imagen del usero que estás a punto de eliminar
    try {
      // Obtén el nombre de la imagen del usero que estás a punto de eliminar
      ...
      const dataSend = { UserId, Name, LastName, Email, UserName, Password, Role, Image:File?(await handleImageUpload(), Image):imageName };
      console.log(Image)
            ...
            <form onSubmit={sendData}>
              ...
              <div className="form-group">
                ...
                <input type="text" readOnly
                  className="form-control" value={userData.UserId} name="id" id="id" aria-describedby="helpId" placeholder="ID" />
              </div>
              <div className="form-group">
                <label htmlFor="Name" className="form-label">Name:</label>
                <input type="text" name="Name" onChange={valueChange} value={userData.Name} id="Name" className="form-control" placeholder="" aria-describedby="helpId" />
               
              </div>
              <div className="form-group">
                <label htmlFor="LastName" className="form-label">LastName:</label>
                <input type="text" name="LastName" value={userData.LastName} onChange={valueChange} id="LastName" className="form-control" placeholder="" aria-describedby="helpId" />
                
              </div>
              <div className="form-group">
                <label htmlFor="Email" className="form-label">Email:</label>
                <input type="text" name="Email" onChange={valueChange} value={userData.Email} id="Email" className="form-control" placeholder="" aria-describedby="helpId" />
                
              </div>
              <div className="form-group">
                <label htmlFor="UserName" className="form-label">UserName:</label>
                <input type="text" name="UserName" value={userData.UserName} onChange={valueChange} id="UserName" className="form-control" placeholder="" aria-describedby="helpId" />
              </div>
              <div className="form-group">
                <label htmlFor="Password" className="form-label">Password:</label>
                <input type="text" name="Password" onChange={valueChange} value={userData.Password} id="Password" className="form-control" placeholder="" aria-describedby="helpId" />
              </div>
              <div className="form-group">
                <label htmlFor="Role" className="form-label">Role:</label>
                <input type="text" name="Role" onChange={valueChange} value={userData.Role} id="Role" className="form-control" placeholder="" aria-describedby="helpId" />
              </div>
              <div className="form-group">
                <input type="file" accept="image/*" onChange={handleImageChange} id="fileInput"/>
                {selectedImage ? (
                  <img src={selectedImage} alt="Preview" style={{ maxWidth: '100px' }} value={userData.Image}/>
                ) : (
                  userData.Image && (
                    <img src={`${Api}/images/${userData.Image}`} alt="User Image" style={{ maxWidth: '100px' }} value={userData.Image}/>
                  )
                )}
